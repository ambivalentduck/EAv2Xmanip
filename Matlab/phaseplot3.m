...
for k=1:lo
    for b=1:length(subject.block)
        subset=subject.block(3).trials(6:end-5);
        if b~=2
...
for k=1:length(learning3)
    figure(2)
    plot(output{1}(subject.block(3).trials(k)), output{2}(subject.block(b).trials(k)))
end 
